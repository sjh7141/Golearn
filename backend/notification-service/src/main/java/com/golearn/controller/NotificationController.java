...
public class NotificationController {
    ...
    @ApiOperation("알림 조회")
    @GetMapping
    public ResponseEntity<List<Notification>> getNotifications(@ApiIgnore @RequestHeader("X-USERNO") int mbrNo) {
        return new ResponseEntity(notificationService.getNotifications(mbrNo), HttpStatus.OK);
    }
    @ApiOperation("알림 갯수 조회")
    @GetMapping("/count")
    public ResponseEntity<CountPayload> getNotificationsCount(@ApiIgnore @RequestHeader("X-USERNO") int mbrNo) {
        return new ResponseEntity(notificationService.getNotificationCount(mbrNo), HttpStatus.OK);
    }
    @ApiOperation("알림 전체 삭제")
    @DeleteMapping
    public ResponseEntity removeNotifications(@ApiIgnore @RequestHeader("X-USERNO") int mbrNo) {
        notificationService.removeNotifications(mbrNo);
        return new ResponseEntity(HttpStatus.OK);
    }
    @ApiOperation("알림 삭제")
    @DeleteMapping("/{noti_no}")
    public ResponseEntity removeNotification(@ApiIgnore @RequestHeader("X-USERNO") int mbrNo, @PathVariable("noti_no") int notiNo) {
